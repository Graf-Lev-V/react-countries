...
export default function Profile() {
    ...
    useEffect(() => {
        ...
        setForm({ name: data.name, email: data.email, phone: data.phone, username: data.username });
    }, [data])
    ...
    function handleSubmit(e) {
        e.preventDefault();
        const newErrors = {};
        Object.entries(form).forEach(([key, value]) => { if (!value) {newErrors[key] = 'Fill Field'; return}; if (key === 'email' && !value.includes('@')) newErrors[key] = 'Fill Email Correctly!'});
        if (Object.keys(newErrors).length === 0) {console.log('Form submitted'); return;}
        setError(newErrors);
    }
